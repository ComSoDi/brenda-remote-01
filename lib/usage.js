...
export function normalizeUsage(raw = {}) {
  ...
  const textInputTokens   = hasInputDetail  ? inputDetail.text   : totalInput;
  const audioInputTokens  = hasInputDetail  ? inputDetail.audio  : 0;
  const textOutputTokens  = hasOutputDetail ? outputDetail.text  : totalOutput;
  const audioOutputTokens = hasOutputDetail ? outputDetail.audio : 0;
...
export function calculateCosts(model, usage) {
  ...
  const total = round6(textInputCost + audioInputCost + textOutputCost + audioOutputCost);
  return {
...
function initBucket(key) {
  return {
    ...
    cost: {
      textInput: 0, audioInput: 0, textOutput: 0, audioOutput: 0, total: 0,
    },
  };
}
...
function addToBucket(bucket, usage, cost) {
  ...
  b.tokens.total         = b.tokens.totalInput + b.tokens.totalOutput;
  ...
  b.cost.total       = round6(b.cost.textInput + b.cost.audioInput + b.cost.textOutput + b.cost.audioOutput);
  return b;
}
...
export async function recordChatUsage({
  db, userId, chatRequestId, callIndex = 0, model, usage, now = new Date(),
}) {
  const normalizedUsage = normalizeUsage(usage);
  const cleanModel = String(model || "").trim().replace(/^models\//, "") || "unknown";
  const cost = calculateCosts(cleanModel, normalizedUsage);
  ...
  const insertRes = await db.collection("gemini_chat_usage_events").updateOne(
    ...
    {
      $setOnInsert: {
        userId, chatRequestId, callIndex, responseId,
        model: cleanModel, usage: normalizedUsage, cost,
        createdAt: now, updatedAt: now,
      },
    },
  ...
  if (inserted) {
    summary = await updateUsageSummary(
      ...
      { userId, model: cleanModel, usage: normalizedUsage, cost, now, sessionId: chatRequestId, responseId }
    );
  }
  return { inserted, usage: normalizedUsage, cost, summary };
}
