...
import { PLAN_ANONYMOUS } from './plans.js';
...
function buildSystemInstructions(locale, gender) {
  ...
  if (locale === "es-ES") {
    return (
      ...
      "No menciones el clima espontáneamente más de una vez por conversación; solo vuelve al tema si el usuario lo pide." +
      genderLine
    );
  }
  ...
  if (locale === "en-GB") {
    return (
      ...
      "Only mention the weather unprompted at most once per conversation; don't bring it up again unless the user asks."
    );
  }
  ...
  try {
    ...
    if (session?.userId && !session.isAnonymous) {
      ...
      if (db) {
        ...
        // must not silently blank out the others, and a failed plan lookup
        // must never be indistinguishable from a genuinely anonymous session.
