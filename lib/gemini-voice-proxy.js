...
export async function createGeminiVoiceProxy(browserWs, req) {
  ...
  // Get gender + medications from session / DB (non-fatal)
  let gender = null;
  let activeMeds = [];
  ...
  try {
    ...
    // non-fatal — continue without gender / meds
  }
  ...
  const systemText = buildSystemInstructions(locale, gender) + buildMedBlock(activeMeds, locale, isAuthenticated);
  ...
  // Gemini → browser: forward first (no latency), then record usage non-blocking
  geminiWs.on("message", (data) => {
    ...
    if (userId) {
      ...
      } catch { /* not JSON or no usageMetadata — ignore */ }
    }
  });
